...
function deviceConnectivityState(device) {
  if (device?.present === false) return 'error'
  const capabilities = ['cellular', 'vowifi']
    .map(key => capability(device, key))
    .filter(item => item.actual !== 'unsupported')
  const desired = capabilities.filter(item => item.desired)
  const relevant = desired.length ? desired : capabilities
  return ['error', 'degraded', 'starting', 'stopping', 'on']
    .find(state => relevant.some(item => item.actual === state)) || 'off'
}

function deviceStatusBadge(device) {
  if (device?.present === false) return { state: 'error', label: 'Offline' }
  const state = deviceConnectivityState(device)
  if (state !== 'off') return { state, label: '' }
  const registration = String(device?.cellular?.registration || '').toLowerCase()
  const flight = capability(device, 'flight')
  if (device?.device_type !== 'reader' && !flight.desired && flight.actual === 'off' &&
      ['home', 'roaming', 'registered'].includes(registration)) {
    return { state: 'on', label: 'Cellular network registered' }
  }
...
function CellularNetworkControl({ device, refreshDevices, showToast }) {
  ...
  useEffect(() => {
    ...
    if (refreshedOperation.current === key) return
    refreshedOperation.current = key
    Promise.resolve(refreshDevices?.()).catch(() => {})
  ...
  if (operatorId && !options.some(item => item.operator_id === operatorId)) {
    ...
    options.unshift({ operator_id: operatorId,
      ...
      name_zh: current?.operator_zh || (saved.operator_id === operatorId && saved.operator_name_zh) || '',
      access_technology: current?.access_technology || saved.access_technology || '', status: 'saved' })
  }
  const observed = currentCellularNetwork(device)
  const updating = Number(operation?.finished_at || 0) > observed.observed_at
  ...
  const apply = async () => {
    ...
    const label = mode === 'automatic' ? t('Automatic network selection')
      : networkLabel(selected || { operator_id: operatorId }, language)
  ...
  return <div className="u-cellular-network">
    <div className="u-cellular-network-heading"><h3>{t('Cellular network selection')}</h3>
      <span className="u-cellular-current">{t('Current network')}: <CellularOperator network={current} connected={current.connected} updating={updating}/></span>
    </div>
    <div className="u-cellular-network-controls">
      ...
      <div className="u-cellular-network-actions">
        ...
        <button className="btn btn-primary"
          disabled={!!busy || unavailable || (mode === 'manual' && (!operatorId || availability(options.find(item => item.operator_id === operatorId)) === 'forbidden'))} onClick={apply}>
    ...
    {mode === 'manual' && <div className="u-cellular-network-list" role="radiogroup" aria-label={t('Available network')}>
      {options.map(item => <button type="button" key={item.operator_id} role="radio"
        ...
        disabled={!!busy || unavailable || availability(item) === 'forbidden'}
        onClick={() => edit({ operatorId: item.operator_id })}>
        <span className="u-network-radio" aria-hidden="true"/>
        <span className="u-network-option-copy"><b title={networkName(item, language)}>{networkName(item, language)}</b>
          {networkName(item, language) !== item.name && item.name && <small className="u-operator-english" title={item.name}>{item.name}</small>}
          <span>{item.operator_id}{item.access_technology ? ` · ${item.access_technology.toUpperCase()}` : ''}</span>
        </span>
        <span className="u-network-availability">{t(availability(item) === 'current' ? 'Current'
          : availability(item) === 'forbidden' ? 'Forbidden' : availability(item) === 'saved' ? 'Saved selection' : 'Detected')}</span>
      </button>)}
...
function CellularOperator({ network, connected, updating = false }) {
  const { t, language } = useI18n()
  if (updating) return <span>{t('Updating registration…')}</span>
  if (!connected) return <span>{t('Not connected')}</span>
  const name = networkName(network, language)
  return <span className="u-cellular-operator">
    <strong>{networkLabel(network, language)}</strong>
    {network.name && name !== network.name && <small className="u-operator-english">{network.name}</small>}
  </span>
}
...
function HardwarePanel({ device, refreshDevices, refresh, showToast }) {
  ...
  const saveName = async () => {
    ...
    if (value.length > 80) { showToast(t('Device name must be at most 80 characters')); return }
    setSaving('name')
    try {
      await api.saveDeviceHardware(device.id, { name: value })
      await refreshDevices()
      showToast(t(value ? 'Device name saved' : 'Default device name restored'))
  ...
  return <div className="card u-panel u-hardware-panel"><div className="u-hardware-intro">
    ...
    <div className="u-details cols u-hardware-facts">
      <div className="u-detail"><span>{t('Device name')}</span><b>{deviceTitle(device, 0, t)}</b></div>
    ...
    <div className="u-hardware-action u-hardware-name">
      <div className="u-hardware-action-copy"><h4>{t('Device name')}</h4>
        <p>{t('This name is used everywhere this physical device appears. Clear it to restore the automatic name.')}</p>
      </div>
      <input maxLength={80} value={name} onChange={event => setName(event.target.value)}
        placeholder={defaultDeviceName(device, 0, t)} />
      <button className="btn btn-primary" disabled={!!saving} onClick={saveName}>{t('Save')}</button>
    </div>
...
export function SystemPage({ showToast }) {
  ...
  const liveTranslation = s.live_translation || { enabled: false, api_key: '', api_key_set: false }
  const translationKeyReady = Boolean(liveTranslation.api_key_set || String(liveTranslation.api_key || '').trim())
  const save = async () => {
    if (!maxSimLinesValid) { showToast(t('SIM line limit must be an integer from 1 to 32.')); return }
    try { const saved = await api.saveSettings(s); setS(saved); showToast(t('Saved')) } catch (e) { showToast(t(e.message)) }
  }
  ...
  return <div className="u-page"><div className="u-tabs">{tabs.map(([k, l]) => <button key={k} className={tab === k ? 'active' : ''} onClick={() => setTab(k)}>{l}</button>)}</div><div className={['backup', 'maintenance'].includes(tab) ? 'u-settings-shell' : 'card u-panel'}>
    {tab === 'general' && <div className="u-settings-form">
      ...
      <section className="u-settings-section">
        ...
        <div className="u-settings-options"><label className="u-settings-option"><input type="checkbox" className="u-toggle" checked={!!s.device_defaults?.cellular_enabled} onChange={e => setS({ ...s, device_defaults: { ...s.device_defaults, cellular_enabled: e.target.checked } })} /><span>{t('Enable cellular data for newly detected modems')}</span></label><label className="u-settings-option"><input type="checkbox" className="u-toggle" checked={s.device_defaults?.vowifi_enabled !== false} onChange={e => setS({ ...s, device_defaults: { ...s.device_defaults, vowifi_enabled: e.target.checked } })} /><span>{t('Enable VoWiFi for newly detected modems')}</span></label><label className="u-settings-option"><input type="checkbox" className="u-toggle" checked={s.device_defaults?.flight_mode !== false} onChange={e => setS({ ...s, device_defaults: { ...s.device_defaults, flight_mode: e.target.checked } })} /><span>{t('Enable flight mode for newly detected modems')}</span></label></div>
        <p className="u-hint">{t('These choices are saved when a cellular modem is first detected. Flight mode turns off its radio before VoWiFi starts; changing these defaults never rewrites known devices.')}</p>
      </section>
      <section className="u-settings-section">
        <h3>{t('Hardware')}</h3>
