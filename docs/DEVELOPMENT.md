...
- 跨页保留只适用于已经提交、由后端持有的异步任务及其结果。未提交的单选候选、批量勾选和
  弹窗操作目标离开所属标签、设备或顶层页面时必须清空，并分别覆盖切标签、切设备和切页回归；
...
上游持续同步的操作步骤、融合取舍和逐次记录见 [上游同步维护目录](upstream-sync/README.md)。
每轮从模板新增一份记录；已确认的历史事实与本轮重新核实的结果分别填写。
...
日常代码修改按 `AGENTS.md` 的默认交付授权完成验证、提交、推送和受管更新；用户明确要求
只读、限制提交/推送/部署或分阶段执行时，以该次要求为准。向权威上游贡献时才遵守其
