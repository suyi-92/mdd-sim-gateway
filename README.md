...
`1.9.4-vmware.38` 修复飞行模式下切换 eSIM 后，已关闭的蜂窝数据显示“正在开启”的问题。
已停用或不可用的 ModemManager 旧卡缓存不再触发身份刷新状态；当前 SIM 绑定和旧卡驻网信息
隔离继续保留，明确的蜂窝恢复等待、初始化进度和失败仍按真实任务呈现。

`1.9.4-vmware.37` 统一概览与设备页的开关说明：设备快照和 Engine 推送交替到达时，已关闭的
VoWiFi 固定显示关闭说明，正常运行固定显示连接说明。后台状态仍展示 Engine 状态，真实启停、
错误、设备离线和蜂窝驻网/信号信息继续实时更新。

`1.9.4-vmware.36` 修复手填手机号保存后的界面同步：保存响应立即更新概览、设备和通话/短信
线路信息，同时推送到其他页面会话；保存前的旧请求不能覆盖新号码。eSIM 通知增加只读缓存
对账，实读确认为空才清除旧通知失败；未尝试投递的读卡器错误在同卡恢复后移到通知区提示核验，
不再把历史读卡失败长期显示在已恢复的配置行，也不把通知读取失败误报成“没有待处理通知”。

`1.9.4-vmware.35` 修复 eSIM 配置行恢复提示停留在旧阶段的问题。实时推送之外，页面同时采用
设备轮询中的同设备、同卡、同任务最新状态；漏掉完成事件后也会在后续设备刷新时自动纠正，
无需重新读取 eUICC 或停止线路。迟到的旧事件不会覆盖新状态，实际失败和拒网结果继续显示。

`1.9.4-vmware.34` 统一手机号码的只读展示格式：概览、设备、通话、短信、线路详情、保号页面和
实时提示都在国家区号与本地号码之间显示一个空格。输入、拨号、API、存储与复制到剪贴板的号码
保持原值；短号、USSD 和非标准号码不做改写。

`1.9.4-vmware.33` 修复号码复制控件的悬停扰动：概览、通话和短信页面中的号码不再调用
浏览器原生大号 tooltip，也不会在鼠标移入时改变颜色、文字装饰或占用尺寸；复制按钮改用常规
指针，点击后的“已复制/复制失败”反馈和键盘焦点提示保持不变。

`1.9.4-vmware.32` 收敛 eSIM profile 行上的持久恢复提示：停用 profile 不再显示早已失效的
`bridge_rebuild_failed`、`operation_timeout` 等恢复结果；当前启用 profile 在同模块、同卡、
桥接就绪且已有更新驻网证据后，也恢复显示正常 ICCID，不再永久展示“曾失败但已恢复”。原始
终态任务继续保留在私有恢复存储中用于审计；当前仍未解决的初始化、拒网、恢复失败以及待处理
通知仍会显示，避免用简单清空掩盖真实故障。

`1.9.4-vmware.31` 修复大疆/Quectel 模块切换 eSIM 后再次失去虚拟读卡器的问题：UICC 的三个
桥接槽只表示并发槽位数，不保证获配的逻辑通道号一定是 1、2、3；基带占用其中一个编号时，第三
个桥接槽可合法获配 4–19。桥接现在按 ETSI 扩展逻辑通道规则编码 CLA，并继续只关闭自己实际
获配的通道，不猜测或抢占基带会话。eSIM 页面同时把终态恢复记录与同模块、同 profile 的更新
桥接/基带/驻网证据合并：保留原失败审计，但不再把已经恢复的线路显示成当前故障。

`1.9.4-vmware.30` 修复受管更新或服务重启后大疆/Quectel 模块的虚拟 eSIM reader 变空：
ModemManager 的 AT 命令可能占用最长 10 秒，编排器不再于 8 秒时强杀桥接，而是在 systemd
总停止预算内等待桥接释放其明确持有的三个 UICC 逻辑通道。若新进程申请到不受三槽桥接支持的
通道，会立即关闭这次由自身申请的通道再失败；仍不扫描或盲关其他进程可能持有的通道。后续
`.31` 已把标准允许的 4–19 号通道纳入支持，只有标准范围之外的返回值才按此规则关闭并失败。

`1.9.4-vmware.29` 区分可恢复后台任务与界面临时选择：蜂窝选网尚未点击“应用网络”时，切换
设备标签、顶层页面或设备后会恢复已保存的自动/手动选网，不再保留未提交的运营商候选；已经
启动的扫描和应用任务仍由后端持有，切页、刷新和返回后继续显示真实进度与结果。通话记录和短信
记录的批量勾选会在离开页面时清空，尚未提交的 eSIM 下载/重命名弹窗也会关闭，避免隐藏页面中的
旧操作目标被误用。

`1.9.4-vmware.28` 修正选网超时后的晚到驻网反馈：选网任务仍保留“截止时超时”的历史终态，
但只要 ModemManager 在任务结束后给出更新且有效的驻网采样，界面就以当前事实替换红色失败指导，
并明确说明请求的自动/手动模式没有被确认保存；手动目标与当前 PLMN 不同则继续给出黄色差异提醒。
蜂窝 `observed_at` 改用实际 `mmcli` 采样完成时间，不再使用可能重发缓存的整份状态文档发布时间，
避免陈旧 `registered` 状态误消除真实失败。

`1.9.4-vmware.27` 将设备能力开关改为可恢复的后台任务：浏览器在目标通过校验并记录后立即获得
任务编号，硬件收敛、线路停止/恢复及失败码由私有任务快照持续发布，切页或刷新不会再把操作变回
空闲。宿主对普通射频、ModemManager 发现和数据承载使用最长 180 秒的 1 秒快速采样窗口，明确
终态后恢复低频轮询；仅切换 VoWiFi 不再等待无关蜂窝状态。全局设备刷新按来源独立更新并合并
尾随刷新，已访问页面保持挂载。设备重检、服务重启和 eSIM 下载会从后端状态恢复进度；eSIM
下载快照不保存激活码、卡身份或原始错误，Control 重启只标记中断，绝不自动重放写卡。飞行模式
等待会暂停 eSIM 蜂窝恢复预算，关闭飞行模式后重新获得有界预算且继续校验 USB/SIM 代次。

`1.9.4-vmware.26` 将 SIM 访问终态按实际路径标注：飞行模式下仅 direct-serial 被执行并失败时
报告 direct 失败；只有先观察到 ModemManager AT 路径 PhoneFailure、随后 direct-serial 也失败，
才报告两条路径均失败。旧版没有路径证据的双路径终态会保守迁移为 direct 单路径；该修订不增加
重试或复位，只纠正诊断证据边界。

`1.9.4-vmware.25` 补齐真实热换 SIM 的桥接代次收敛：一个桥接已直接证明卡身份后，若后续
读到另一张已验证卡，立即退役旧逻辑通道；若卡身份连续三轮不可验证，也将该代次标为错误并
退出，由编排器有界重建。单次 AT/APDU 争用仍允许恢复，不把一次慢读当成换卡；退役时只关闭
本进程确实分配的通道。这样旧 Engine/SWu/IMS 状态不能在换卡后继续冒充当前卡。

`1.9.4-vmware.24` 将模块 eSIM 切换拆成可核验事务：lpac 请求 REFRESH 后，桥接卡身份与
基带 SIM 身份分别确认；飞行模式中不能完成的基带初始化会持久等待，在关闭飞行模式后先按
同一 USB/卡代次补齐，再恢复射频和自动选网。读卡 8 秒只表示仍在处理，硬截止由独立进程
安全终止；主动重检等待该次 SIM 读取结束，不再启动重叠 APDU 或提前报告完成。

该版本还为 MM/direct-serial 双路径失败增加有界终态，直接 AT 射频写入必须回读确认，删除
不具备型号证据的通用 CFUN 重启。ModemManager 1.23 的 QMI 网络拒绝日志只提取受控字段并按
USB、SIM 与服务代次绑定；LTE/EPS 原因码 7 显示为网络服务拒绝，与 SIM 可读状态和普通超时
分开。eSIM 通知反馈保留阶段、分类、次数和耗时，未知错误不再猜测；VID/PID `2c7c:0125`
的默认名称改为通用 DJI/Quectel 蜂窝模块，不再据此断言具体 EC25 型号。

`1.9.4-vmware.23` 修复切卡恢复的额外轮询等待：使用本轮处理前的输入快照检测新请求，
避免处理过程中到达的请求被吞掉唤醒；桥接恢复未结束时等待上限为 1 秒，完成或失败后
恢复正常空闲频率。保留 SIM、ModemManager 和新桥接身份一致后才确认成功的门禁。

`1.9.4-vmware.22` 将模块切卡的 eSIM 通知处理移到桥接就绪之后，PC/SC 临时占用最多尝试
三次，共用 45 秒执行预算。失败保留在对应配置文件下，重新打开页面仍可查看；切卡成功和
通知处理结果分别呈现。切卡完成的全槽实卡核验交给监测器复用，减少同一桥接和维护窗口内
的重复读卡；新桥接、实际拔插、后续 LPA 操作和主动重检仍执行身份检查。

`1.9.4-vmware.21` 将选网及失败恢复限制在统一 180 秒预算内，显示请求、确认、恢复阶段和
剩余时间。eSIM 每次启用后默认自动选网并清除手动运营商。国际号码按模块上报的号码类型恢复
`+`，分别展示 eSIM 服务商、号码地区和 SIM 归属网；号码地区不会改写真实 MCC/MNC 或出口配置。

`1.9.4-vmware.20` 在 eSIM 切换完成后恢复新卡保存的自动/手动选网设置，避免继承上一张卡
留在模块里的手动 PLMN；复用后台选网进度与结果，切页后仍可查看。

`1.9.4-vmware.19` 移除会使 DJI 兼容模块停在 offline 的通用 Reset。切卡后仅对已确认的
QMI SIM 槽做供电刷新，再定向重新探测 ModemManager；必须与实卡身份一致才能报告恢复完成。

`1.9.4-vmware.18` 修复 `454-00` 等合法零 MNC 被误判为空、导致 ePDG 地址缺失的问题。
eSIM 切换后以当前 bridge 的实卡身份校准线路；蜂窝层仍缓存旧卡时只刷新对应模块，等待
新 SIM 身份与桥接均确认。通话/短信选择跟随同一设备的新卡，VoWiFi 关闭时明确显示关闭，
不因启用配置文件而擅自启动线路。

`1.9.4-vmware.17` 按精确 PLMN 统一中国移动、联通和电信的访问网络名称，避免模块遗留名称
与新编号混用；界面以中文为主、英文小字备注。操作结果与当前驻网分开呈现，旧的扫描恢复
警告只在更新的驻网证据确认恢复后解除，连接断开或变化时不再沿用“已连接”提示。

`1.9.4-vmware.16` 将蜂窝扫描与选网改为独立后台任务，切页或刷新后恢复进度、运营商名称和结果，
并拒绝操作未结束时的重复扫描。扫描成功但驻网恢复未完成时保留网络列表并单独提示恢复状态。
扫描按钮始终显示，自动模式下禁用；单设备重检移到设备标签栏旁。默认验证后提交、推送并
通过 `mddctl update` 更新的交付约定已写入 `AGENTS.md`。

`1.9.4-vmware.15` 为手动选网增加实际 PLMN 驻网核验、超时后的有界等待和失败恢复，
Quectel QMI 模块通过受管 AT 选网并同步 ModemManager，仅在模式和驻网均确认后保存选择；
失败显示中文原因及恢复状态。选网区使用紧凑模式切换和运营商列表，
完整检测收进设备列表标题，单设备检测缩为行内按钮，反馈复用说明位置，不再留下空白槽。

`1.9.4-vmware.14` 修复 Quectel QMI 扫描返回空列表后被显示为 `unknown error` 的问题：
根据 ModemManager 实时端口选择同一模块的标准 AT 扫描；可准确恢复选网方式时临时退网，
成功或失败后均恢复原自动/数字手动选网，保留完整扫描超时和选网前置检查。
全设备检测移到设备列表底部，单设备重检移入“硬件”页，使用次要按钮与固定反馈区域。

`1.9.4-vmware.13` 在全局完整检测之外新增单设备重新检测：蜂窝模块仅重建目标 bridge、
清理目标缓存并启动 ModemManager 扫描，原生读卡器仅触发 PC/SC 重新枚举；蜂窝网络扫描在
ModemManager 尚未恢复时会明确提示先重检设备，不再无说明地禁用按钮。
