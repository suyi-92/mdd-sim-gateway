...
- 本仓库采用用户授予的默认交付授权：没有特殊要求时，每次代码修改并通过所需验证后，
  默认完成提交、推送 `origin/vmware`，再执行 `sudo mddctl update --yes` 和上线健康检查，
  不停留在补丁或提交阶段，也无需逐轮重复询问。
- 本轮明确提出的“只读”“不要提交”“不要推送”“不要更新/部署”、限定范围或快照前置要求
  优先于默认授权；已有未提交工作仍须确认来源并保留。默认授权不允许跳过测试、Git 门禁、
  失败回滚或隐私检查，不包含强推、破坏性清理和未经请求的硬件固件/身份写入。
...
- 开始同步前阅读 `docs/upstream-sync/README.md`、`WORKFLOW.md` 和最近一轮记录，逐项复核
  上轮未合入、部分合入及未验证事项；不能把历史通过结果直接沿用到本轮。
- 复用 `docs/upstream-sync/` 作为唯一合并日志目录。每轮从 `TEMPLATE.md` 新建
  `YYYY-MM-DD-vX.Y.Z.md`，同日同版本多轮时增加序号；更新 `README.md` 索引，不覆盖旧轮次。
- 先记录原 VMware SHA/版本、上次上游目标、本次 fetch 后核对的 `origin/main` 与
  `upstream/main`、最终固定目标完整 SHA/版本、集成分支、worktree 和授权范围。
- 按功能建立清单，明确标注“完整合入、部分合入、已有等价实现、明确不适用、暂缓、确认遗漏”；
  每项关联上游提交、实际代码路径、处理理由和验证结果。不能把提交数量当成功能数量，也不能
  把不同实现或上次目标之后才出现的修复误记为上次漏合。
- 所有未完整覆盖事项必须写明缺少什么、影响、未合入原因、后续处理和验收条件；明确取舍也要
  保留依据。说明 VMware 自有功能是否保留、哪些重复修复已收敛，以及冲突解决的实际决策。
- 合并、测试、构建、提交、推送及部署过程中持续更新本轮记录。仅填写实际执行结果；分别记录
  自动测试、产物身份、VM 运行与硬件/通话验收，未执行写“未验证”，失败写阶段与恢复结果。
- 日志须跟随对应源码变更交付；合并完成后的 SHA 和后续验收可用普通后续提交补记，不为让日志
  引用自身 SHA 而改写历史。未授权提交、推送或部署时标为待执行，记录要求不扩大操作授权。
- 日志只保存脱敏结论与可复核依据；原始配置、凭据、卡身份、号码和消息正文不得入库。
...
- 模块桥接已证明卡身份后，确认新身份或连续身份丢失必须退役旧桥接代次并有界重建；旧 Engine、
  PIN、SWu 或 IMS 文件不得在真实换卡后继续充当当前卡证据。清理仍只限本进程明确持有的通道。
