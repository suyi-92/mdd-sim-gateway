...
class InstallerContractTests(unittest.TestCase):
    ...
    def test_active_generation_is_raw_canonical_and_identity_checked(self):
        ...
        self.assertIn('bash "$BOOTSTRAP_VERIFY_INSTALLER" verify', validator)
        self.assertIn('"$BOOTSTRAP_VERIFY_INSTALLER" == "$INSTALL_DIR/install.sh"', validator)
        self.assertIn('! -f "$BOOTSTRAP_VERIFY_INSTALLER" || -L "$BOOTSTRAP_VERIFY_INSTALLER"', validator)
        self.assertIn('-e "$MDDCTL_SOURCE_ROOT/.git"', MDDCTL)
...
class VersionContractTests(unittest.TestCase):
    def test_vmware_version_suffix(self):
        ...
        self.assertEqual(version, "1.9.4-vmware.10")
