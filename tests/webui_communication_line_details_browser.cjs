  ...
  try {
    ...
    const verifyFirstLine = async view => {
      ...
      for (const expected of ['运营商', '线路名称', '号码', '号码地区', 'SIM 归属网', '网络线路',
        'Desk line', '+44 7700900357', '英国 (GB)', 'GB Fixture Node']) {
    ...
    await callHistory.getByText('+1 2025550123', { exact: true }).waitFor()
    ...
    await page.locator('.u-message-thread-list').getByText('+1 2025550123', { exact: true }).click()
    ...
    assert.equal(await overviewNumber.innerText(), '+44 7700900357')
    ...
    assert.equal(await sailyDetails.getByRole('button', { name: '复制号码' }).innerText(), '+1 2025550123')
    for (const width of [1440, 900, 390]) {
      await page.setViewportSize({ width, height: 900 })
      assert.equal(await page.locator('.u-content').evaluate(element => element.scrollWidth > element.clientWidth), false)
